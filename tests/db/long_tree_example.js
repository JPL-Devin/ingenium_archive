...
function add_section(i, max_count, id_prefix, target_id, level, prepend) {
  ...
  return fns.addElement(id_execution_1, elem, target_id, level, prepend)
  .then(
    function() {
      return add_section(i+1, max_count, id_prefix, target_id, level, prepend);
    }
  );
}

function add_step(i, max_count, id_prefix, target_id, level, prepend) {
  console.log('add_section i: %s max_count: %s', i, max_count);
  ...
  .then(
    function() {
      return add_step(i+1, max_count, id_prefix, target_id, level, prepend);
    }
  );
}
...
.then((res) => {
    return add_section(0, size, 'id_section_', '', 'CHILD', true);
  }
)
/*
.then((res) => {
...
.then(()=>{
  ...
  return fns.getAsRun('execution/' + id_execution_1, 'long_as_run_created.json');
})
.then((res)=>{
    console.info('Done: ');
