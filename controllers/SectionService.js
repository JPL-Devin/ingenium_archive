...
exports.create_section = async function(args, res, next) {
  /**
   * Create a section
   *
  ...
  try {
    ...
    const data = await node_funcs.addElement(execution_id, section, insert_after_id, level);
    res.status(200).json(data);
  } catch (err) {
    const err_data = base_funcs.push_error('Error when adding a section', err);
    res.status(400).json(err_data);
  }
}

exports.get_execution_sections = async function(args, res, next) {
  /**
  ...
  try {
    ...
    const err_data = base_funcs.push_error('Error when getting elements', err);
    res.status(400).json(err_data);
  }
}

exports.get_section = async function(args, res, next) {
  /**
  ...
  try {
    ...
    const err_data = base_funcs.push_error('Error when getting a section', err);
    res.status(400).json(err_data);
  }
}

exports.update_section = async function(args, res, next) {
  /**
   * Update a section
   *
   * elem_id String unique id of a procedure element
  ...
  try {
    const data = await node_funcs.updateElement(elem_id, section);
    res.status(200).json(data);    
  } catch (err) {
    const err_data = base_funcs.push_error('Error when updating a section', err);
    res.status(400).json(err_data);
  }
}
